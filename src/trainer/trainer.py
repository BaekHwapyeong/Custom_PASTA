...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        use_fmri = False,  # New parameter for fMRI usage
        use_fc = False,    # New parameter for FC usage
    ):
        super().__init__()
        ...
        if dataset == 'MRI2PET':
            if not eval_mode:
                ...
                self.ds = MRI2PET_2_5D_Dataset(self.image_size, data_path = train_data, output_dim=input_slice_channel,
                    ...
                    use_fmri = use_fmri, use_fc = use_fc)
    ...
    def _extract_data_components(self, data):
        """
        Extract data components based on whether fMRI and FC are enabled
        Returns: mri_data, pet_data, data_label, tabular_data, slice_index, loss_weight_mask, mri_uid, fmri_data, fc_data
        """
        mri_data, pet_data = data[0], data[1]
        data_label = data[2]
        tabular_data = data[3]
        slice_index = data[4]
        loss_weight_mask = data[5]
        mri_uid = data[6]
        ...
        current_idx = 7
        if self.use_fmri:
            fmri_data = data[current_idx]
            current_idx += 1
        if self.use_fc:
            fc_data = data[current_idx]
            current_idx += 1
            
        return mri_data, pet_data, data_label, tabular_data, slice_index, loss_weight_mask, mri_uid, fmri_data, fc_data

    
    def _extract_test_data_components(self, test_data):
        """Extract components from test data tuple"""
        if len(test_data) >= 8:  # mri, pet, label, tabular, slice_index, loss_weight_mask, mri_uid, pet_uid
            test_data_mri, test_data_pet, dx_label, test_tabular_data, test_slice_index, test_loss_weight_mask, test_mri_uid, test_pet_uid = test_data[:8]
            test_fmri_data = test_data[8] if len(test_data) > 8 else None
            test_fc_data = test_data[9] if len(test_data) > 9 else None
        elif len(test_data) >= 6:  # SlicedScanMRI2PETDataset format
            test_data_mri, test_data_pet, dx_label, test_tabular_data, test_mri_uid, test_pet_uid = test_data[:6]
            test_slice_index = None
            test_loss_weight_mask = None
            test_fmri_data = test_data[6] if len(test_data) > 6 else None
            test_fc_data = test_data[7] if len(test_data) > 7 else None
        else:
            # fallback for old format
            test_data_mri, test_data_pet, dx_label, test_tabular_data, test_mri_uid = test_data[:5]
            test_pet_uid = test_mri_uid  # fallback
            test_slice_index = None
            test_loss_weight_mask = None
            test_fmri_data = None
            test_fc_data = None
        
        return test_data_mri, test_data_pet, dx_label, test_tabular_data, test_mri_uid, test_pet_uid, test_fmri_data, test_fc_data
                ...
                for _ in range(self.gradient_accumulate_every):
                    ...
                    # with self.accelerator.autocast():
                        if self.dataset == 'MRI2PET' or self.dataset == 'GM2PET':
                            ...
                            mri_data, pet_data, data_label, tabular_data, slice_index, loss_weight_mask, mri_uid, fmri_data, fc_data = self._extract_data_components(data)
                            ...
                            # Prepare additional conditioning data
                            additional_cond = {}
                            if self.use_fmri and fmri_data is not None:
                                additional_cond['fmri'] = fmri_data.to(device)
                            if self.use_fc and fc_data is not None:
                                additional_cond['fc'] = fc_data.to(device)

                            model_output = self.model(pet_data, cond = cond_data, tab_cond = tabular_data,
                                                    loss_weight_mask = loss_weight_mask, **additional_cond)
                    ...
                    if self.step != 0 and self.step % self.save_and_sample_every == 0:
                        ...
                        if self.dataset == 'MRI2PET':
                            ...
                            for valid_data in self.dl_valid:
                                # Extract validation data components
                                valid_data_mri, valid_data_pet, valid_data_label, valid_tabular_data, valid_slice_index, valid_loss_weight_mask, valid_mri_uid, valid_fmri_data, valid_fc_data = self._extract_data_components(valid_data)
                                
                                valid_cond_data = valid_data_mri

                                if self.tabular_cond:
                                    tabular_data = valid_tabular_data.to(device)
                                else: 
                                    tabular_data = None
                                
                                # Prepare additional conditioning data for validation
                                additional_cond = {}
                                if self.use_fmri and valid_fmri_data is not None:
                                    additional_cond['fmri'] = valid_fmri_data.to(device)
                                if self.use_fc and valid_fc_data is not None:
                                    additional_cond['fc'] = valid_fc_data.to(device)
                                ...
                                with torch.no_grad():
                                    ...
                                    loss = self.ema.ema_model(valid_data_pet.to(device), cond = valid_cond_data.to(device), 
                                                        tab_cond = tabular_data, **additional_cond)['loss'].mean()
                                    valid_loss = loss
                                    ...
                                    sample_pet = self.ema.ema_model.sample(shape = valid_data_pet.shape, cond = valid_cond_data.to(device), 
                                                        tab_cond = tabular_data, **additional_cond)
    ...
    @torch.no_grad()
    def evaluate(self, checkpoint, evaluate_folder, synthesis=False, synthesis_folder=None, get_ROI_loss=True):
        ...
        if self.dataset == 'MRI2PET':
            ...
            for test_data in self.dl_test:
                ...
                test_data_mri, test_data_pet, dx_label, test_tabular_data, test_mri_uid, test_pet_uid, test_fmri_data, test_fc_data = self._extract_test_data_components(test_data)
                if self.tabular_cond:
                    tabular_data = test_tabular_data.to(self.device)
                else:
                    tabular_data = None

                # Prepare additional conditioning data for testing
                additional_cond = {}
                if self.use_fmri and test_fmri_data is not None:
                    additional_cond['fmri'] = test_fmri_data.to(self.device)
                if self.use_fc and test_fc_data is not None:
                    additional_cond['fc'] = test_fc_data.to(self.device)
                    ...
                    with torch.no_grad():                            
                        ...
                        if self.model_cycling and not synthesis:
                            model_output = self.ema.ema_model(cond_slice, cond = sample_pet, tab_cond = tabular_data, 
                                                    model_cycle = True, **additional_cond)
                ...
                for slice_batch in range(whole_pet_sample.shape[0]):
                    ...
                    _pet_filename = str(test_pet_uid[slice_batch]).replace('.npy','')  # PET 파일명 사용

                    if synthesis:
                        _L1_loss = np.abs(whole_pet_sample[slice_batch] - whole_GT_pet[slice_batch])
                        L1_loss = _L1_loss.mean()
                        
                        # PET 파일명 + 예측 label + _syn_pet 형식으로 저장
                        pet_sample_file_name = str(syn_folder / f'{_pet_filename}_{img_label}_syn_pet.nii.gz')
                        ...
                        print(f'MRI -> PET: {_pet_filename}_{img_label}: {L1_loss:.6f}')
                        with open(str(syn_folder / f'all_eval_scores.txt'), 'a') as f:
                            f.write('\n')
                            f.write(f'MRI -> PET: {_pet_filename}_{img_label}: {L1_loss:.6f}')
                        
                        if get_ROI_loss:
                            ROI_loss = _L1_loss * self.ROI_mask
                            ROI_loss = ROI_loss.mean()
                            print(f'MRI -> PET ROI loss: {_pet_filename}_{img_label}: {ROI_loss:.6f}')
                            with open(str(syn_folder / f'all_eval_scores.txt'), 'a') as f:
                                f.write('\n')
                                f.write(f'MRI -> PET ROI loss: {_pet_filename}_{img_label}: {ROI_loss:.6f}')
                            roi_losses.append(ROI_loss)
                    else:
                        # evaluation 모드에서도 동일하게 적용
                        whole_pet_sample_img = nib.Nifti1Image(whole_pet_sample[slice_batch].squeeze(), affine=affine)
                        whole_pet_sample_img.to_filename(str(self.evaluate_folder / f'whole_sample_pet_{_pet_filename}_{slice_batch}_{img_label}.nii.gz'))

                        whole_GT_pet_img = nib.Nifti1Image(whole_GT_pet[slice_batch].squeeze(), affine=affine)
                        whole_GT_pet_img.to_filename(str(self.evaluate_folder / f'whole_GT_pet_{_pet_filename}_{slice_batch}_{img_label}.nii.gz'))

                        error_map_img = nib.Nifti1Image((whole_pet_sample[slice_batch] - whole_GT_pet[slice_batch]).squeeze(), affine=affine)                     
                        error_map_img.to_filename(str(self.evaluate_folder / f'error_map_{_pet_filename}_{slice_batch}_{img_label}.nii.gz'))
                    ...
                    if self.model_cycling and not synthesis:
                        ...
                        print(f'PET -> MRI: {_pet_filename}_{img_label}: {L1_loss_mri:.6f}')
                        print('====================')
                        with open(str(self.evaluate_folder / f'eval_scores.txt'), 'a') as f:
                            f.write('\n')
                            f.write(f'PET -> MRI: {_pet_filename}_{img_label}: {L1_loss_mri:.6f}')
                            f.write('\n')
                            f.write('====================')
